...
const PayPrompt = () => {
  ...
  return (
    <div>
          ...
          <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
            ...
            <form onSubmit={handlePay} className="space-y-4">
              ...
              <Button type="submit" disabled={loading} className="w-full">
                {loading ? 'Processing...' : 'Proceed with STK Push'}
              </Button>
              <Button
                type="button"
                variant="outline"
                ...
                onClick={() => {
                  const mpesaUrl = `mpesa://paybill?business=${qrData.businessShortCode}&amount=${amount || ''}&account=${qrData.reference || ''}`;
      ...
      {/* Optionally, show a background or info page here */}
    </div>
  );
};
